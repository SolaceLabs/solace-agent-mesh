...
class CrossDatabaseInspector:
    ...
    async def verify_agent_schema_state(self, agent_name: str):
        ...
        assert "alembic_version" not in table_names, f"Agent {agent_name} should not have migration table"
        assert "agent_sessions" in table_names, f"Agent {agent_name} missing required tables"
        assert "agent_messages" in table_names, f"Agent {agent_name} missing required tables"
    ...
    async def get_gateway_sessions(self, user_id: str) -> List[SessionRecord]:
        ...
        with gateway_engine.connect() as conn:
            result = conn.execute(
                text("SELECT id, user_id, agent_name, created_at FROM gateway_sessions WHERE user_id = :user_id"),
                {"user_id": user_id}
            )
    ...
    async def get_agent_sessions(self, agent_name: str, gateway_session_id: Optional[str] = None) -> List[AgentSessionRecord]:
    ...
    async def get_agent_messages(self, agent_name: str, gateway_session_id: str) -> List[MessageRecord]:
    ...
    async def verify_session_linking(self, gateway_session_id: str, agent_name: str):
        ...
        assert gateway_session is not None, f"Gateway session {gateway_session_id} not found"
        assert agent_session is not None, f"Agent session for {gateway_session_id} not found in {agent_name}"
        assert agent_session[1] == gateway_session_id, f"Session ID mismatch in agent database"
        assert agent_session[2] == agent_name, f"Agent name mismatch in agent database"
        assert gateway_session[1] == agent_session[3], f"User ID mismatch between Gateway and Agent"
        
        return {
            "gateway_session": SessionRecord(gateway_session[0], gateway_session[1], gateway_session[2], ""),
    ...
    async def verify_database_isolation(self, agent_a: str, agent_b: str):
        ...
        for session in agent_a_sessions:
            ...
            assert agent_b_session is None, f"Session leak detected: {session[0]} found in both {agent_a} and {agent_b} databases"
            
        return True
        
    async def verify_session_context_isolation(self, session_x_id: str, session_y_id: str, agent_name: str):
        ...
        assert content_x != content_y, f"Session content appears to be identical between {session_x_id} and {session_y_id}"
