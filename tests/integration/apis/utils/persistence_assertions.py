...
from ..infrastructure.database_inspector import (
...
async def assert_gateway_session_exists(
    ...
    assert len(matching_sessions) == 1, f"Expected 1 gateway session with ID {session_id}, found {len(matching_sessions)}"
    
    session = matching_sessions[0]
    assert session.user_id == user_id, f"Session user_id mismatch: expected {user_id}, got {session.user_id}"
    assert session.agent_name == agent_name, f"Session agent_name mismatch: expected {agent_name}, got {session.agent_name}"
...
async def assert_agent_session_exists(
    ...
    assert len(sessions) == 1, f"Expected 1 agent session for {gateway_session_id}, found {len(sessions)}"
    
    session = sessions[0]
    assert session.gateway_session_id == gateway_session_id, f"Gateway session ID mismatch"
    assert session.agent_name == agent_name, f"Agent name mismatch"
    assert session.user_id == user_id, f"User ID mismatch"
...
async def assert_session_message_count(
    ...
    assert len(messages) == expected_count, f"Expected {expected_count} messages, found {len(messages)}"
...
async def assert_agent_session_message_count(
    ...
    assert len(messages) == expected_count, f"Expected {expected_count} agent messages, found {len(messages)}"
...
async def assert_message_content_contains(
    ...
    assert len(matching_messages) > 0, f"No messages found containing '{expected_content}'"
...
async def assert_database_isolation(
    ...
    assert isolation_verified, f"Database isolation violated between {agent_a} and {agent_b}"
...
async def assert_session_linking(
    ...
    linking_result = await inspector.verify_session_linking(gateway_session_id, agent_name)
    ...
    assert agent_session.gateway_session_id == gateway_session_id, "Agent session linking mismatch"
...
async def assert_migration_state(
    ...
    if expected_version:
        assert migration_version == expected_version, f"Expected migration version {expected_version}, got {migration_version}"
...
async def assert_agent_schema_correct(
    ...
    for table in expected_tables:
        assert table in table_names, f"Agent {agent_name} missing expected table: {table}"
        
    # Ensure no migration table exists
    assert "alembic_version" not in table_names, f"Agent {agent_name} should not have alembic_version table"
...
async def assert_session_context_isolation(
    ...
    assert isolation_result["content_isolated"], f"Sessions {session_a_id} and {session_b_id} do not have isolated content"
    assert isolation_result["session_x_messages"] > 0, f"Session {session_a_id} has no messages"
    assert isolation_result["session_y_messages"] > 0, f"Session {session_b_id} has no messages"
...
async def assert_database_stats(
    ...
    for db_name, expected_counts in expected_stats.items():
        ...
        for stat_name, expected_count in expected_counts.items():
            ...
            assert actual_count == expected_count, f"{db_name}.{stat_name}: expected {expected_count}, got {actual_count}"


async def assert_no_data_leakage(
    inspector: CrossDatabaseInspector,
    user_a_sessions: List[str],
    user_b_id: str
):
    ...
    for session_id in user_a_sessions:
        assert session_id not in user_b_session_ids, f"User A session {session_id} found in User B's data"