...
def test_send_streaming_task(api_client: TestClient):
    ...
    task_data = {
        "agent_name": "TestAgent",
        "message": "Start streaming conversation"
    }
...
def test_send_task_to_existing_session(api_client: TestClient):
    ...
    initial_response = api_client.post("/api/v1/tasks/subscribe", data=initial_task_data)
    ...
    followup_response = api_client.post("/api/v1/tasks/subscribe", data=followup_task_data)
