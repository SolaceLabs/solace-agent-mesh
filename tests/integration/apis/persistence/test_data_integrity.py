...
def test_session_deletion_cascades_to_messages(api_client: TestClient):
    ...
    for message in additional_messages:
        ...
        followup_response = api_client.post("/api/v1/tasks/subscribe", data=followup_data)
    ...
    message_contents = [msg["message"] for msg in history if msg["sender_type"] == "user"]
...
@pytest.mark.skip(reason="Requires multi-user test architecture - both TestClients authenticate as same user due to AuthMiddleware limitations")
def test_cross_user_data_isolation_comprehensive(api_client: TestClient, test_app):
    ...
    def create_second_user_client():
        ...
        second_component.get_app.return_value = Mock(app_config={
        ...
        second_component._translate_external_input = AsyncMock(return_value=(
            "TestAgent", [], {}
        ))
        
        # Different user authentication
        second_component.authenticate_and_enrich_user = AsyncMock(return_value={
    ...
    try:
        ...
        user1_messages = [msg["message"] for msg in user1_history.json() if msg["sender_type"] == "user"]
        ...
        print(f"✓ Data isolation verified: User 1 has {len(user1_session_ids)} sessions, User 2 has {len(user2_session_ids)} sessions")
...
def test_orphaned_data_prevention(api_client: TestClient):
    ...
    orphan_response = api_client.post("/api/v1/tasks/subscribe", data=orphan_attempt_data)
...
def test_referential_integrity_with_multiple_deletions(api_client: TestClient):
    ...
    for session_id, expected_count in deletion_order[:3]:  # Delete first 3
        ...
        remaining_sessions = [(sid, count) for sid, count in remaining_sessions if sid != session_id]
        ...
        for remaining_id, remaining_count in remaining_sessions:
            ...
            remaining_history = api_client.get(f"/api/v1/sessions/{remaining_id}/history")
            assert remaining_history.status_code == 200
            remaining_messages = remaining_history.json()
            user_messages = [msg for msg in remaining_messages if msg["sender_type"] == "user"]
    ...
    print(f"✓ Referential integrity maintained through {len(deleted_sessions)} deletions")
...
def test_session_consistency_across_operations(api_client: TestClient):
    ...
    update_response = api_client.patch(f"/api/v1/sessions/{session_id}", json={"name": "Consistency Test Session"})
    ...
    target_session = next((s for s in sessions_data["sessions"] if s["id"] == session_id), None)
...
def test_data_integrity_under_concurrent_operations(api_client: TestClient):
    ...
    for i in range(3):
        ...
        update_response = api_client.patch(f"/api/v1/sessions/{session_id}", json=update_data)
    ...
    assert successful_ops == total_ops, f"Only {successful_ops}/{total_ops} operations succeeded"
    ...
    print(f"✓ Data integrity maintained through {total_ops} operations on session {session_id}")
...
def test_user_data_cleanup_integrity(api_client: TestClient):
    ...
    for i in range(4):
        task_data = {
            "agent_name": f"TestAgent" if i % 2 == 0 else "TestPeerAgentA",
