...
class TestBasicCRUDOperations:
    ...
    def test_create_new_task(self, gateway_adapter: GatewayAdapter, database_inspector: DatabaseInspector):
        """Verify that a new task can be created via the gateway adapter."""
        session = gateway_adapter.create_session(user_id="crud_user", agent_name="TestAgent")
    ...
    def test_retrieve_tasks_for_session(self, gateway_adapter: GatewayAdapter, database_inspector: DatabaseInspector):
        """Verify that tasks can be retrieved for a session."""
        session = gateway_adapter.create_session(user_id="retrieval_user", agent_name="TestAgent")
    ...
    def test_update_existing_task_upsert(self, api_client: TestClient, gateway_adapter: GatewayAdapter):
        ...
        original_payload = {
            ...
            "taskMetadata": json.dumps({"status": "in_progress", "agent_name": "TestAgent"})
        }
        
        first_response = api_client.post(f"/api/v1/sessions/{session.id}/chat-tasks", json=original_payload)
        ...
        second_response = api_client.post(f"/api/v1/sessions/{session.id}/chat-tasks", json=updated_payload)
    ...
    def test_empty_session_returns_empty_array(self, api_client: TestClient, gateway_adapter: GatewayAdapter):
...
class TestAuthorizationAndSecurity:
    ...
    def test_user_can_only_access_own_session_tasks(self, api_client: TestClient, gateway_adapter: GatewayAdapter):
    ...
    def test_task_isolation_between_sessions(self, gateway_adapter: GatewayAdapter, database_inspector: DatabaseInspector):
        ...
        session_b = gateway_adapter.create_session(user_id="sam_dev_user", agent_name="TestAgent")
...
class TestIntegrationWithExistingFeatures:
    ...
    def test_tasks_cascade_delete_with_session(self, api_client: TestClient, gateway_adapter: GatewayAdapter, database_inspector: DatabaseInspector):
    ...
    def test_messages_endpoint_derives_from_tasks(self, api_client: TestClient, gateway_adapter: GatewayAdapter):
    ...
    def test_feedback_updates_task_metadata(self, api_client: TestClient, gateway_adapter: GatewayAdapter):
...
class TestDataValidation:
    ...
    def test_large_payload_handling(self, gateway_adapter: GatewayAdapter, database_inspector: DatabaseInspector):
