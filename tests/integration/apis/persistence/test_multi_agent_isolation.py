...
def test_agent_database_complete_isolation(
    ...
    simple_gateway_adapter.send_message(
        session_main.id,
        "Message for TestAgent"
    )
    
    simple_gateway_adapter.send_message(
        session_peer_a.id,
        "Message for PeerAgentA"
    )
    
    simple_gateway_adapter.send_message(
        session_peer_b.id,
        "Message for PeerAgentB"
    )
    
    # Verify database isolation between all agent pairs
    assert simple_database_inspector.verify_database_isolation("TestAgent", "TestPeerAgentA")
    assert simple_database_inspector.verify_database_isolation("TestAgent", "TestPeerAgentB")
    assert simple_database_inspector.verify_database_isolation("TestPeerAgentA", "TestPeerAgentB")
...
def test_agent_session_context_isolation(
    ...
    simple_gateway_adapter.send_message(
        session_a.id,
        "Working on context A task"
    )
    
    simple_gateway_adapter.send_message(
        session_b.id,
        "Working on context B task"
    )
    ...
    print(f"✓ Agent session context isolation verified: {session_a.id} ≠ {session_b.id}")
...
def test_agent_database_schema_isolation(
    ...
    for agent_name in test_agents_list:
        ...
        for expected_table in expected_tables:
            assert expected_table in table_names, f"Agent {agent_name} missing table {expected_table}"
    ...
    print(f"✓ Schema isolation verified: {len(test_agents_list)} agents have identical schemas")


@pytest.mark.parametrize("agent_name", ["TestAgent", "TestPeerAgentA", "TestPeerAgentB"])
def test_individual_agent_isolation(
    ...
    simple_gateway_adapter.send_message(
        session.id,
        f"Message for {agent_name}"
    )
    
    # Verify this agent's session exists in Gateway database
    user_sessions = simple_database_inspector.get_gateway_sessions(f"user_for_{agent_name.lower()}")
...
def test_concurrent_agent_operations(
    ...
    session_peer_a = simple_gateway_adapter.create_session("concurrent_user", "TestPeerAgentA") 
    session_peer_b = simple_gateway_adapter.create_session("concurrent_user", "TestPeerAgentB")
...
def test_agent_message_content_isolation(
    ...
    for agent_name, content in agents_and_content:
        session = simple_gateway_adapter.create_session(
            user_id="content_isolation_user",
            agent_name=agent_name
        )
