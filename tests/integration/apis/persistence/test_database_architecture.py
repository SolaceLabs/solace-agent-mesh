...
def test_agent_databases_have_direct_schema(
    ...
    print(f"✓ Agent databases ({len(test_agents_list)}): Direct schema creation (no migrations)")
...
def test_initial_database_state(
    simple_database_inspector: SimpleDatabaseInspector
):
...
def test_database_connections_work(
    ...
    print(f"✓ All database connections working: Gateway + {len(test_agents_list)} agents")