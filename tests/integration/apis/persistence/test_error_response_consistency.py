    ...
    for method, endpoint, *data in security_test_cases:
        ...
        for term in sensitive_terms:
            if term in ["exist", "permission", "unauthorized", "forbidden", "user"]:
                ...
                assert term not in error_detail, f"Error message contains sensitive term '{term}': {error_detail}"
...
def test_error_response_structure_validation(api_client: TestClient):
    ...
    for test_case in error_test_cases:
        ...
        if "jsonrpc" in error_data:
            ...
            assert "error" in error_data, "Missing 'error' field in JSON-RPC error response"
            assert error_data["error"] is not None
            assert "message" in error_data["error"], "Missing 'message' in JSON-RPC error"
        else:
            # Standard HTTP error format
            for field in expected_fields:
                assert field in error_data, f"Missing required field '{field}' in error response"
                assert error_data[field] is not None
                assert len(str(error_data[field])) > 0
        
        # Ensure no internal/debug fields are exposed
        internal_fields = [
            "traceback", "stack_trace", "exception", "debug", 
            "internal_error", "sql", "database", "file_path"
        ]
        
        for internal_field in internal_fields:
            assert internal_field not in error_data, f"Internal field '{internal_field}' exposed in error response"
    ...
    for test_data in unicode_test_cases:
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                pytest.fail(f"Error response with unicode data failed to decode: {response.content}")
