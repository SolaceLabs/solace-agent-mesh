...
class TestOpenApiToolConfig:
    ...
    def test_openapi_config_with_tool_name_prefix(self):
        """Test valid config with tool_name_prefix."""
        config = {
            "tool_type": "openapi",
            "specification_file": "./api.json",
            "tool_name_prefix": "api1"
        }
        result = OpenApiToolConfig.model_validate(config)
        assert result.tool_type == "openapi"
        assert result.tool_name_prefix == "api1"

    def test_openapi_config_without_tool_name_prefix(self):
    ...
    def test_openapi_config_prefix_with_other_fields(self):
        """Test tool_name_prefix works with other fields."""
        config = {
            "tool_type": "openapi",
            "specification_file": "./api.json",
        ...
        result = OpenApiToolConfig.model_validate(config)
        assert result.tool_name_prefix == "petstore"
        assert result.base_url == "https://api.example.com"
        assert result.allow_list == ["getPets"]

    def test_openapi_config_prefix_invalid_type(self):
