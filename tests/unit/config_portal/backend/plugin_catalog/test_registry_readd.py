...
class TestAPIEndpointIntegration:
    """Integration tests for the API endpoint with re-add functionality."""
    
    def test_add_registry_api_returns_is_update_flag(self):
        """Test that the API endpoint returns is_update flag."""
        from plugin_catalog_server import create_plugin_catalog_app
        
        app = create_plugin_catalog_app()
        client = app.test_client()
        
        with patch('plugin_catalog_server.registry_manager') as mock_manager:
            with patch('plugin_catalog_server.plugin_scraper') as mock_scraper:
                # Mock successful add (not update)
                mock_manager.add_registry.return_value = (True, False)
                mock_manager.get_all_registries.return_value = []
                mock_scraper.get_all_plugins.return_value = []
                
                response = client.post(
                    '/api/plugin_catalog/registries',
                    json={'path_or_url': 'https://github.com/test/repo.git'},
                    content_type='application/json'
                )
                
                assert response.status_code == 200
                data = json.loads(response.data)
                assert 'is_update' in data
                assert data['is_update'] is False
    
    def test_add_registry_api_clears_cache_on_update(self):
        """Test that API clears git cache when re-adding a registry."""
        from plugin_catalog_server import create_plugin_catalog_app
        ...
        with patch('plugin_catalog_server.registry_manager') as mock_manager:
            with patch('plugin_catalog_server.plugin_scraper') as mock_scraper:
                ...
                mock_manager.add_registry.return_value = (True, True)
                mock_manager._generate_registry_id.return_value = "test_id"
                mock_manager.get_all_registries.return_value = []
                mock_scraper.get_all_plugins.return_value = []
                ...
                assert response.status_code == 200
                # Verify clear_git_cache was called
                mock_scraper.clear_git_cache.assert_called_once_with("test_id")
                # Verify get_all_plugins was called with force_fresh_clone=True
                mock_scraper.get_all_plugins.assert_called_once()
                call_args = mock_scraper.get_all_plugins.call_args
                assert call_args[1]['force_fresh_clone'] is True
