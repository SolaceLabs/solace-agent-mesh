...
class TestPublicShareAccess:
    ...
    def test_authenticated_user_can_access_public_share(self):
        link = make_share_link(require_authentication=False)
        can_access, _ = link.can_be_accessed_by_user(
            user_id="some-user", user_email="user@example.com"
        )
...
class TestAuthRequiredShareAccess:
    ...
    def test_authenticated_user_without_domain_restriction_is_allowed(self):
        ...
        can_access, _ = link.can_be_accessed_by_user(
            user_id="uid", user_email="user@any.com"
        )
        assert can_access is True
...
class TestDomainRestrictedShareAccess:
    def test_user_from_allowed_domain_is_permitted(self):
        ...
        can_access, _ = link.can_be_accessed_by_user(
            user_id="uid", user_email="alice@company.com"
        )
        assert can_access is True
    ...
    def test_user_from_different_domain_is_denied(self):
        ...
        can_access, reason = link.can_be_accessed_by_user(
            user_id="uid", user_email="alice@other.com"
        )
        assert can_access is False
    ...
    def test_unauthenticated_user_denied_even_with_matching_domain(self):
        ...
        can_access, reason = link.can_be_accessed_by_user(
            user_id=None, user_email="alice@company.com"
        )
        assert can_access is False
        assert reason == "authentication_required"
    ...
    def test_multiple_allowed_domains_any_one_grants_access(self):
        link = make_share_link(
            require_authentication=True, allowed_domains="company.com,partner.org"
        )
        can_access_company, _ = link.can_be_accessed_by_user(
            user_id="u1", user_email="a@company.com"
        )
        can_access_partner, _ = link.can_be_accessed_by_user(
            user_id="u2", user_email="b@partner.org"
        )
        can_access_other, _ = link.can_be_accessed_by_user(
            user_id="u3", user_email="c@other.io"
        )
    ...
    def test_domain_restriction_requires_authentication_flag(self):
        ...
        link = make_share_link(
            require_authentication=False, allowed_domains="company.com"
        )
        ...
        can_access2, _ = link.can_be_accessed_by_user(
            user_id="uid", user_email="alice@company.com"
        )
...
class TestUserSpecificShareAccess:
    ...
    def test_listed_user_is_allowed(self):
        ...
        can_access, _ = link.can_be_accessed_by_user(
            "uid-alice", "alice@example.com", shared_emails
        )
        assert can_access is True
    ...
    def test_unlisted_authenticated_user_is_denied(self):
        ...
        can_access, reason = link.can_be_accessed_by_user(
            "uid-carol", "carol@example.com", shared_emails
        )
        assert can_access is False
    ...
    def test_unauthenticated_user_denied_when_user_specific_sharing_active(self):
        ...
        link = make_share_link(
            require_authentication=False
        )  # would be public without shared list
        shared_emails = ["alice@example.com"]
    ...
    def test_empty_shared_users_list_falls_back_to_general_rules(self):
        ...
        can_access_unauth, reason = link.can_be_accessed_by_user(
            None, None, shared_user_emails=[]
        )
        assert can_access_unauth is False
        assert reason == "authentication_required"
        # Authenticated user with empty shared list falls back to general rules (allowed)
        can_access_auth, _ = link.can_be_accessed_by_user(
            "uid", "user@example.com", shared_user_emails=[]
        )
    ...
    def test_user_specific_check_is_case_insensitive(self):
        ...
        shared_emails = ["Alice@Example.COM"]
        can_access, _ = link.can_be_accessed_by_user(
            "uid", "alice@example.com", shared_emails
        )
        assert can_access is True
...
class TestViewSharedSessionEndpoint:
    ...
    async def test_returns_401_when_authentication_required_but_not_provided(
        ...
        mock_share_service.get_shared_session_view = AsyncMock(
            side_effect=PermissionError(
                "Authentication required to view this shared session"
            )
        )
    ...
    async def test_returns_403_when_domain_does_not_match(
        ...
        mock_share_repo.find_by_share_id.return_value = make_share_link(
            user_id="other-owner"
        )
    ...
    @pytest.mark.asyncio
    async def test_returns_404_when_share_not_found(self, mock_db, mock_share_service):
        from fastapi import HTTPException
...
class TestGetSharedArtifactEndpoint:
    ...
    async def test_unauthenticated_user_denied_on_auth_required_share(self, mock_db):
        ...
        from solace_agent_mesh.gateway.http_sse.routers.share import (
            get_shared_artifact_content,
        )
    ...
    async def test_unlisted_user_denied_on_user_specific_share(self, mock_db):
        ...
        with (
            patch(
                "solace_agent_mesh.gateway.http_sse.repository.share_repository.ShareRepository",
                return_value=self._make_share_repo(
                    share_link, shared_emails=["alice@example.com"]
                ),
            ),
            pytest.raises(HTTPException) as exc_info,
        ):
...
class TestShareServiceAccessEnforcement:
    ...
    def _make_service(self):
        from solace_agent_mesh.gateway.http_sse.services.share_service import (
            ShareService,
        )
    ...
    async def test_raises_permission_error_when_auth_required_and_not_provided(self):
        ...
        with pytest.raises(PermissionError, match="Authentication required"):
            await service.get_shared_session_view(
                MagicMock(), "abc", user_id=None, user_email=None
            )
    ...
    async def test_user_specific_share_allows_listed_user(self):
        ...
        with (
            ...
            mock_session_repo.find_user_session.return_value = MagicMock(
                project_id=None
            )
...
class TestCreateShareLinkBusinessRules:
    ...
    def test_domain_restriction_without_auth_required_is_rejected(self):
        ...
        with patch(
            ...
            MockSessionService.return_value.get_session_details.return_value = (
                mock_session
            )
            ...
            with pytest.raises(
                ValueError, match="Domain restrictions require authentication"
            ):
                service.create_share_link(
                    MagicMock(), "sess1", "user1", request, "http://app"
                )
...
class TestShareUserManagementPersistence:
    ...
    def test_add_users_commits_transaction(self):
        ...
        service.repository.add_share_user.return_value = SharedLinkUser(
            id="slu-123",
            share_id="share123",
            user_email="alice@example.com",
            ...
            original_access_level=None,
            original_added_at=None,
        )
        mock_db = MagicMock()

        result = service.add_share_users(
            mock_db, "share123", "owner", [{"user_email": "alice@example.com"}]
        )

        # Verify repository was called to add the user
        service.repository.add_share_user.assert_called_once()
    ...
    def test_remove_users_commits_transaction(self):
        ...
        result = service.delete_share_users(
            mock_db, "share123", "owner", ["alice@example.com"]
        )

        # Verify repository was called to delete users
        service.repository.delete_share_users_batch.assert_called_once()
        # Verify operation succeeded
    ...
    def test_non_owner_cannot_add_users(self):
        ...
        with pytest.raises(ValueError, match="Not authorized"):
            service.add_share_users(
                MagicMock(), "share123", "attacker", ["victim@example.com"]
            )
    ...
    def test_non_owner_cannot_remove_users(self):
        ...
        with pytest.raises(ValueError, match="Not authorized"):
            service.delete_share_users(
                MagicMock(), "share123", "attacker", ["alice@example.com"]
            )
