...
class TestSSEManagerContextManager:
    """Test context manager functionality (if implemented)."""
    
    def test_context_manager_not_implemented(self):
        """Test that SSEManager doesn't implement context manager by default."""
        # Setup
        event_buffer = MagicMock(spec=SSEEventBuffer)
        manager = SSEManager(100, event_buffer)
        
        # Verify context manager methods don't exist
        assert not hasattr(manager, '__aenter__')
        assert not hasattr(manager, '__aexit__')


class TestSSEManagerIntegration:
    """Integration tests combining multiple SSEManager features."""

    @pytest.mark.asyncio
    async def test_full_lifecycle_integration(self):
        """Test complete lifecycle: create connections, send events, cleanup."""
        # Setup
        event_buffer = MagicMock(spec=SSEEventBuffer)
        event_buffer.get_and_remove_buffer.return_value = None
        manager = SSEManager(100, event_buffer)
        ...
        # Phase 2: Send events
        event_data1 = {"message": "first event", "id": 1}
        event_data2 = {"message": "second event", "id": 2}
        
        await manager.send_event(task_id, event_data1, "message")
        await manager.send_event(task_id, event_data2, "update")
        
        # Phase 3: Verify events received
        assert queue1.qsize() == 2
        assert queue2.qsize() == 2
        
        # Phase 4: Remove one connection
        await manager.remove_sse_connection(task_id, queue1)
        
        # Phase 5: Send another event
        event_data3 = {"message": "third event", "id": 3}
        await manager.send_event(task_id, event_data3, "message")
        
        # Phase 6: Verify only remaining connection got the event
        assert queue1.qsize() == 2  # No new events
        assert queue2.qsize() == 3  # Got the new event
    ...
    @pytest.mark.asyncio
    async def test_buffering_and_flushing_integration(self):
        """Test event buffering and flushing when connections are created."""
        # Setup
        event_buffer = MagicMock(spec=SSEEventBuffer)
        buffered_events = [
            {"event": "message", "data": "buffered1"},
            {"event": "message", "data": "buffered2"}
        ]
        event_buffer.get_and_remove_buffer.return_value = buffered_events
        
        manager = SSEManager(100, event_buffer)
        task_id = "buffering-task"
        
        # Send event before any connections exist
        await manager.send_event(task_id, {"message": "new event"}, "message")
        
        # Verify event was buffered
        event_buffer.buffer_event.assert_called_once()
        
        # Create connection - should flush buffered events
        queue = await manager.create_sse_connection(task_id)
        
        # Verify buffered events were flushed to the new connection
        assert queue.qsize() == 2
        event1 = await queue.get()
        event2 = await queue.get()
        assert event1 == {"event": "message", "data": "buffered1"}
        assert event2 == {"event": "message", "data": "buffered2"}
