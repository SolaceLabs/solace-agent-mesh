...
def test_cross_user_session_access_returns_404(multi_user_test_setup):
    ...
    print(f"User A identity: {user_a_me.status_code} - {user_a_me.json() if user_a_me.status_code == 200 else user_a_me.text}")
    
    user_b_me = second_user_client.get("/api/v1/users/me")
    print(f"User B identity: {user_b_me.status_code} - {user_b_me.json() if user_b_me.status_code == 200 else user_b_me.text}")
    
    # User A creates a session
    task_data = {
        "agent_name": "TestAgent",
        "message": "User A's private session"
    }
    ...
    print(f"User A sessions: {sessions_a.status_code} - {sessions_a.json() if sessions_a.status_code == 200 else sessions_a.text}")
    
    # Verify User A can access their own session
    session_response = first_user_client.get(f"/api/v1/sessions/{session_id}")
    print(f"User A session access: {session_response.status_code} - {session_response.text}")
    ...
    print(f"User B unauthorized access: {unauthorized_response.status_code} - {unauthorized_response.text}")
...
def test_cross_user_session_history_returns_404(multi_user_test_setup):
    ...
    task_data = {
        "agent_name": "TestAgent",
        "message": "User A's private conversation"
    }
    response_a = first_user_client.post("/api/v1/tasks/subscribe", data=task_data)
    assert response_a.status_code == 200
    session_id = response_a.json()["result"]["sessionId"]
    ...
    unauthorized_history = second_user_client.get(f"/api/v1/sessions/{session_id}/history")
    ...
    print(f"✓ Cross-user session history access properly returns 404 for session {session_id}")
...
def test_cross_user_session_update_returns_404(multi_user_test_setup):
    ...
    update_response = first_user_client.patch(f"/api/v1/sessions/{session_id}", json=update_data)
    ...
    unauthorized_update = second_user_client.patch(f"/api/v1/sessions/{session_id}", json=malicious_update)
    ...
    assert verify_response.json()["name"] == "User A's Updated Session"  # Should still be User A's name
    
    print(f"✓ Cross-user session update properly blocked with 404 for session {session_id}")
...
def test_cross_user_session_deletion_returns_404(multi_user_test_setup):
    ...
    print(f"✓ Cross-user session deletion properly blocked with 404 for session {session_id}")
...
def test_session_isolation_in_listing(multi_user_test_setup):
    ...
    for i in range(3):
        task_data = {
            "agent_name": "TestAgent",
            "message": f"User A's session {i+1}"
        }
    ...
    for i in range(2):
        task_data = {
            "agent_name": "TestAgent",
            "message": f"User B's session {i+1}"
        }
    ...
    print(f"✓ Session isolation verified: User A has {len(user_a_session_ids)} sessions, User B has {len(user_b_session_ids)} sessions")


def test_consistent_404_for_nonexistent_and_unauthorized_sessions(multi_user_test_setup):
    ...
    real_patch_response = second_user_client.patch(f"/api/v1/sessions/{real_session_id}", json=update_data)
    fake_patch_response = second_user_client.patch(f"/api/v1/sessions/{fake_session_id}", json=update_data)
    
    assert real_patch_response.status_code == 404
    assert fake_patch_response.status_code == 404
    
    # Test DELETE endpoints
    real_delete_response = second_user_client.delete(f"/api/v1/sessions/{real_session_id}")
    fake_delete_response = second_user_client.delete(f"/api/v1/sessions/{fake_session_id}")
...
def test_session_ownership_after_multiple_operations(multi_user_test_setup):
    ...
    task_data = {
        "agent_name": "TestAgent",
        "message": "Multi-operation test session"
    }
    response_a = first_user_client.post("/api/v1/tasks/subscribe", data=task_data)
    assert response_a.status_code == 200
    session_id = response_a.json()["result"]["sessionId"]
    ...
    update_response = first_user_client.patch(f"/api/v1/sessions/{session_id}", json={"name": "Updated Name"})
    assert update_response.status_code == 200
    ...
    followup_response = first_user_client.post("/api/v1/tasks/subscribe", data=followup_task)
    ...
    assert second_user_client.get(f"/api/v1/sessions/{session_id}/history").status_code == 404
    assert second_user_client.patch(f"/api/v1/sessions/{session_id}", json={"name": "Hijack"}).status_code == 404
    assert second_user_client.delete(f"/api/v1/sessions/{session_id}").status_code == 404
    ...
    print(f"✓ Session ownership consistently maintained across multiple operations for session {session_id}")