...
def test_multi_agent_consultation_workflow(api_client: TestClient):
    ...
    for session_id, (agent_name, _) in zip(session_ids, agent_consultations):
    ...
    for session_id, (agent_name, initial_message) in zip(session_ids, agent_consultations):
    ...
    for session_id, name in zip(session_ids, session_names):
...
def test_session_management_workflow(api_client: TestClient):
    ...
    for created_session in sessions_created:
        found_session = next(s for s in sessions_data["sessions"] if s["id"] == created_session["id"])
...
def test_error_recovery_workflow(api_client: TestClient):
    ...
    task_data = {
        "agent_name": "TestAgent",
        "message": "Normal conversation start"
    }
    ...
    assert response.status_code in [400, 422, 500]  # Various error responses are acceptable
...
def test_high_volume_workflow(api_client: TestClient):
    ...
    print(f"✓ High volume workflow successful: {len(session_ids)} sessions with {len(session_ids) * 4} total messages")