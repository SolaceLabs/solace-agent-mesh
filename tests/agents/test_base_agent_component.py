...
class TestAction(Action):
    ...
    def __init__(self, **kwargs):
        attributes = {
            ...
            "examples": [{"param1": "test", "param2": 42}],
        }
...
class TestBaseAgentComponent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_connector.get_command_control_service.return_value = (
            self.mock_command_control
        )
        ...
        self.agent = TestAgent(
            ...
            config={
                ...
                "component_config": {"registration_interval": 60},
            },
            flow_name="test_flow",
            component_index=0,
            index=0,
    ...
    def test_invoke_successful(
        self, mock_middleware_service_class, mock_file_service_class
    ):
        """Test successful action invocation."""
        # Set up mocks
        mock_file_service = MagicMock()
        mock_file_service_class.return_value = mock_file_service
        mock_file_service.resolve_all_resolvable_urls.return_value = {
            "param1": "test",
            "param2": 42,
        }
        ...
        self.assertEqual(
            result["topic"],
            "test/solace-agent-mesh/v1/actionResponse/agent/test_agent/test_action",
        )

        # Check that stats were updated
        self.assertEqual(self.agent.action_stats["test_action"]["total_invocations"], 1)
        self.assertEqual(
            self.agent.action_stats["test_action"]["successful_invocations"], 1
        )
        self.assertEqual(
            self.agent.action_stats["test_action"]["failed_invocations"], 0
        )
        self.assertIsNotNone(
            self.agent.action_stats["test_action"]["last_execution_time_ms"]
        )

    @patch("src.services.file_service.FileService")
    @patch("src.services.middleware_service.middleware_service.MiddlewareService")
    def test_invoke_with_file(
        self, mock_middleware_service_class, mock_file_service_class
    ):
    ...
    def test_invoke_error(self, mock_middleware_service_class, mock_file_service_class):
        ...
        self.assertEqual(self.agent.action_stats["test_action"]["total_invocations"], 1)
        self.assertEqual(
            self.agent.action_stats["test_action"]["successful_invocations"], 0
        )
        self.assertEqual(
            self.agent.action_stats["test_action"]["failed_invocations"], 1
        )
    ...
    def test_handle_timer_event(self):
        ...
        self.assertEqual(
            sent_message.get_topic(),
            "test/solace-agent-mesh/v1/register/agent/test_agent",
        )
    ...
    def test_handle_update_agent_config(self):
        ...
        result = self.agent._handle_update_agent_config(
            body={"description": "Updated description", "always_open": False}
        )
    ...
    def test_handle_patch_agent_config(self):
        ...
        # Test with valid data
        result = self.agent._handle_patch_agent_config(
            body={"description": "Patched description", "custom_field": "patched_value"}
        )
    ...
    def test_handle_update_full_agent_config(self):
        ...
        # Test with valid data
        result = self.agent._handle_update_full_agent_config(
            body={
                "agent": {
                ...
                "component": {"registration_interval": 120},
            }
        )
    ...
    def test_handle_get_agent_actions(self):
        ...
        self.assertEqual(
            result[0]["description"], "This is a test action for unit tests"
        )
    ...
    def test_handle_get_action_stats(self):
        ...
        result = self.agent._handle_get_action_stats(
            query_params={"action_name": "test_action"}
        )
        ...
        result = self.agent._handle_get_action_stats(
            query_params={"action_name": "nonexistent_action"}
        )
    ...
    def test_handle_get_action_stats_by_name(self):
        ...
        # Test with invalid action
        result = self.agent._handle_get_action_stats_by_name(
