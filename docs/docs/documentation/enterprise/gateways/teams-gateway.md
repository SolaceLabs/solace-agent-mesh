...
This guide walks you through configuring the Microsoft Teams Gateway after deploying it in Solace Agent Mesh (SAM). It covers both **SAM Cloud** and **SAM Kubernetes** environments.
...
5. Navigate to **Configuration**
   - **Messaging endpoint**: Set to a placeholder for now (e.g., `https://placeholder.com/api/messages`) -- you will update this in Step 4 after obtaining your gateway URL
...
After saving, **deploy the gateway** from the SAM UI. The gateway pod and its networking resources (Service, etc.) are only created after deployment.
...
3. Copy this full URL -- you will configure it in Azure Bot Service in the next step
...
In SAM Kubernetes, the platform creates a **ClusterIP Service** for the gateway, making it accessible only within the Kubernetes cluster. You are responsible for exposing the endpoint to the public internet so that Microsoft Teams can reach it.
...
The actual service name and port are automatically configured by the platform during deployment.
...
To verify your setup, open `https://<your-public-domain>/health` in a browser. You should see a JSON health response from the gateway, confirming the endpoint is publicly accessible.
...
Create a file named `manifest.json` using the template below. Replace `<YOUR_APP_ID>` with the **Application (client) ID** from Step 1, and update the developer information with your organization's details.
