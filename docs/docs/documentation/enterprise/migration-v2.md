...
| **Agent Mesh Doctor Pre-flight Checks** | Pre-install/pre-upgrade Helm hook that validates broker, database, object storage, TLS, and OIDC configuration before any workload pods are created. Misconfigurations surface as a clear error instead of `CrashLoopBackOff`. Enabled by default (`samDoctor.enabled: true`); requires the enterprise image to include `sam_doctor`. See [Agent Mesh Doctor](./production-kubernetes.md#agent-mesh-doctor-pre-flight-validation). | `samDoctor.enabled` |
