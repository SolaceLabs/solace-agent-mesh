...
to the Skill Learning Service's internal queue.
"""
...
info = {
    "class_name": "SkillLearningForwarderComponent",
    "description": (
        "Forwards messages from an internal BrokerInput to the Skill Learning "
        "Service's internal queue for processing."
    ),
    "config_parameters": [
        {
            ...
            "required": True,
            "type": "queue.Queue",
            "description": "A direct reference to the target queue.Queue instance.",
        }
    ],
    "input_schema": {
        "type": "object",
...
class SkillLearningForwarderComponent(ComponentBase):
    """
    ...
    def __init__(self, **kwargs: Any):
        ...
        log.info("%s SkillLearningForwarderComponent initialized.", self.log_identifier)
    ...
    def invoke(self, message: SolaceMessage, data: Dict[str, Any]) -> None:
        ...
        log.info(
            "%s Received message from broker. Topic: %s, Data keys: %s",
            log_id_prefix,
            data.get("topic"),
        ...
        try:
            ...
            try:
                ...
                log.warning(
                    "%s Skill learning queue is full. Message dropped. Current size: %d",
                    log_id_prefix,
            ...
            log.exception(
                "%s Error in SkillLearningForwarderComponent invoke: %s",
                log_id_prefix,
                e,
            )
            if message:
