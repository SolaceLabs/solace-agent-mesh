...
from ...business.domain.project_domain import ProjectCopyRequest
...
async def get_global_project_templates(
    ...
    """
    try:
        projects = await project_service.get_global_projects()
        
        # Get usage counts for each template
        template_responses = []
        for project in projects:
            usage_count = await project_service.get_template_usage_count(project.id)
...
async def copy_project_from_template(
    ...
    try:
        ...
        project = await project_service.copy_project_from_template(copy_request)
