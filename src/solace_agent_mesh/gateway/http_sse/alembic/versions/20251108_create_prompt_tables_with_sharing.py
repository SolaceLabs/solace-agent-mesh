...
def _upgrade_mysql() -> None:
    ...
    op.create_table(
        ...
        sa.Column('is_pinned', sa.Boolean(), nullable=False, server_default=sa.text('0')),  # MySQL needs numeric default
        sa.Column('created_at', sa.BigInteger(), nullable=False),
        sa.Column('updated_at', sa.BigInteger(), nullable=False),
        sa.ForeignKeyConstraint(
            ['production_prompt_id'],
...
def downgrade() -> None:
    """Remove all prompt-related tables (handle circular FK dependencies)."""
    bind = op.get_bind()
    ...
    # Circular FK: prompts ↔ prompt_groups
    # Need to handle circular dependencies differently per dialect

    if bind.dialect.name == 'mysql':
        # MySQL: Drop FK constraints first, then tables
        # Skip index drops (FK-protected, auto-removed with table)
        op.drop_constraint('fk_prompts_group_id', 'prompts', type_='foreignkey')
        op.drop_constraint('prompt_groups_ibfk_1', 'prompt_groups', type_='foreignkey')  # FK from prompt_groups to prompts
        op.drop_table('prompts')
        op.drop_table('prompt_groups')
    elif bind.dialect.name == 'postgresql':
        # PostgreSQL: Drop indexes then use CASCADE
        op.drop_index('ix_prompts_user_id', table_name='prompts')
        op.drop_index('ix_prompts_group_id', table_name='prompts')
        bind.execute(sa.text('DROP TABLE prompts CASCADE'))
