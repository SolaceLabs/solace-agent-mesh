...
async def run_tool_async(
    ...
    if ctx_param_name or artifact_params:
        ...
        # Check if the function's first param looks like a legacy context
        # receiver: not in the LLM-provided args and has no type annotation.
        # Simple tool functions (e.g. def hello(name: str)) should NOT get
        # a context wrapper injected — only legacy sandbox tools that
        # explicitly declare an un-annotated first param for context.
        sig = inspect.signature(func)
        params = [
            p for p in sig.parameters.values()
            if p.name not in ("self", "cls", "kwargs")
        ]
        first_param = params[0] if params else None
        needs_legacy_ctx = (
            first_param is not None
            and first_param.name not in args
