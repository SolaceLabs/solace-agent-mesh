...
class SandboxWorkerComponent(SamComponentBase):
    ...
    def _get_broker_input(self) -> Optional[BrokerInput]:
        """Get the BrokerInput from this component's flow for dynamic subscriptions."""
        if hasattr(self, "_broker_input_ref") and self._broker_input_ref is not None:
            return self._broker_input_ref

        app = self.get_app()
        if app and hasattr(app, "flows") and app.flows:
            flow = app.flows[0]
            if flow.component_groups and flow.component_groups[0]:
                candidate = flow.component_groups[0][0]
                if isinstance(candidate, BrokerInput):
                    self._broker_input_ref = candidate
                    return candidate

        return None

    def subscribe(self, topic: str) -> bool:
        """Dynamically subscribe to a broker topic."""
        broker_input = self._get_broker_input()
        if broker_input is None:
            log.error(
                "%s Cannot subscribe to '%s': BrokerInput not available yet",
                self.log_identifier,
                topic,
            )
            return False
        return broker_input.add_subscription(topic)

    def unsubscribe(self, topic: str) -> bool:
        """Dynamically unsubscribe from a broker topic."""
        broker_input = self._get_broker_input()
        if broker_input is None:
            log.error(
                "%s Cannot unsubscribe from '%s': BrokerInput not available yet",
                self.log_identifier,
                topic,
            )
            return False
