...
class ToolManifest:
    ...
    def ensure_packages_installed(self) -> None:
        """Install any missing packages declared in the manifest and local wheels."""
        for entry in self._entries.values():
            if entry.package:
                if not self._is_package_installed(entry.package):
                    self._install_package(entry.package, entry.version)

        self._install_local_wheels()

    def install_pending_wheels(self) -> None:
        """Install any wheel files that haven't been installed yet.

        Safe to call on every poll iteration — skips already-installed wheels.
        """
        self._install_local_wheels()

    def _install_local_wheels(self) -> None:
        """Discover and install .whl files from the wheels/ directory next to the manifest."""
        manifest_dir = Path(self._path).parent
        wheels_dir = manifest_dir / "wheels"
        if not wheels_dir.is_dir():
            return

        for whl_path in sorted(wheels_dir.glob("*.whl")):
            if whl_path.name in self._installed_wheels:
                continue
            log.info("Installing local wheel: %s", whl_path.name)
            self._install_wheel(whl_path)
            self._installed_wheels.add(whl_path.name)

    def _install_wheel(self, whl_path: Path) -> None:
        """Install a single .whl file using uv pip install (falls back to pip)."""
        for installer in [["uv", "pip", "install"], ["pip", "install"]]:
            try:
                result = subprocess.run(
                    [*installer, str(whl_path)],
                    capture_output=True,
                    text=True,
                    timeout=120,
                )
                if result.returncode == 0:
                    log.info("Successfully installed wheel: %s", whl_path.name)
                    return
                else:
                    log.warning(
                        "Wheel install failed with %s: %s",
                        installer[0],
                        result.stderr[:500],
                    )
                ...
                log.error("Wheel install timed out: %s", whl_path.name)
                return

        log.error("Failed to install wheel %s (no installer available)", whl_path.name)
