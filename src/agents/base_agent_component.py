...
from ..services.llm_service.components.llm_service_component_base import (
    LLMServiceComponentBase,
)
...
class BaseAgentComponent(LLMServiceComponentBase, ABC):
    ...
    def invoke(self, message, data):
        ...
        with self.create_trace_context(
            ...
            trace_level="INFO",
            entity_type="agent",
            entity_id=self.info.get("agent_name"),
        ) as trace_ctx:
            ...
            if not action_name:
                ...
                self._update_action_stats(
                    action_name, False, start_time, "Action name not provided"
                )
            else:
                ...
                if not action:
                    ...
                    self._update_action_stats(
                        action_name, False, start_time, "Action not found"
                    )
                else:
                    ...
                    try:
                        ...
                        trace_ctx.progress(
                            data={"resolved_params": "File URLs resolved successfully"}
                        )

                        middleware_service = MiddlewareService()
                        if middleware_service.get("base_agent_filter")(
                            message.user_properties or {}, action
                        ):
                            try:
                                meta = {
                                    "session_id": session_id,
                                ...
                                if action_response.inline_files:
                                    file_names.extend(
                                        [f.name for f in action_response.inline_files]
                                    )
                                ...
                                self._update_action_stats(
                                    action_name, False, start_time, str(e)
                                )
                            ...
                            # Update action stats for failed invocation
                            self._update_action_stats(
                                action_name, False, start_time, "Unauthorized access"
                            )
                        ...
                        self._update_action_stats(
                            ...
                            False,
                            start_time,
                            f"Error resolving file URLs: {str(e)}",
                        )
    ...
    def _update_action_stats(
        self, action_name: str, success: bool, start_time: float, error: str = None
    ) -> None:
        ...
        total_time = stats["average_execution_time_ms"] * (
            stats["total_invocations"] - 1
        )
        stats["average_execution_time_ms"] = (total_time + execution_time_ms) / stats[
            "total_invocations"
        ]
    ...
    def register_with_command_control(self) -> None:
        ...
        if not self.connector or not hasattr(
            ...
            log.debug(
                "Command and control service not available, skipping registration"
            )
            return
        ...
        if success:
            log.info(
                f"Agent {self.info['agent_name']} registered with command and control system"
            )
        else:
            log.warning(
                f"Failed to register agent {self.info['agent_name']} with command and control system"
            )
            ...
            endpoints.append(
                {
                    ...
                    "methods": {
                        "GET": {
                            ...
                            "handler": lambda path_params=None, query_params=None, body=None, context=None, action_name=action_name: self._handle_get_action_info(
            ...
            endpoints.append(
                {
                    ...
                    "methods": {
                        "GET": {
                            ...
                            "handler": lambda path_params=None, query_params=None, body=None, context=None, action_name=action_name: self._handle_get_action_stats_by_name(
                                path_params, query_params, body, context, action_name
                            ),
                        }
                    },
    ...
    def _get_command_control_configuration(self) -> Dict[str, Any]:
        ...
        return {
            "current_config": {"component": filtered_config, "agent": agent_config},
            "mutable_paths": mutable_paths,
            "config_schema": {
                "type": "object",
                "properties": {
                    "agent": {"type": "object", "properties": schema_properties}
                },
            },
        }
    ...
    def _handle_update_agent_config(
        self, path_params=None, query_params=None, body=None, context=None
    ):
    ...
    def _handle_patch_agent_config(
        self, path_params=None, query_params=None, body=None, context=None
    ):
    ...
    def _handle_update_full_agent_config(
        self, path_params=None, query_params=None, body=None, context=None
    ):
            ...
            for key, value in body["agent"].items():
                if key != "agent_name":  # Don't allow changing the agent name
                    if key in self.info:
                        ...
                        if isinstance(self.info[key], bool) and not isinstance(
                            value, bool
                        ):
                            if value.lower() in ["true", "yes", "1"]:
                                value = True
                        ...
                        if isinstance(self.info[key], int) and not isinstance(
                            value, int
                        ):
                            try:
                                value = int(value)
                        ...
                        if isinstance(self.info[key], float) and not isinstance(
                            value, float
                        ):
                            try:
                                value = float(value)
    ...
    def _handle_get_agent_actions(
        ...
        for action in self.action_list.actions:
            actions.append(
                {"name": action.name, "description": action.long_description}
            )
        return actions

    def _handle_get_action_info(
        self,
        path_params=None,
        query_params=None,
        body=None,
    ...
    def _handle_get_action_stats_by_name(
        self,
        path_params=None,
        query_params=None,
        body=None,
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        for action_name, stats in self.action_stats.items():
            ...
            total_execution_time += (
                stats["average_execution_time_ms"] * stats["total_invocations"]
            )
            action_count += 1
        ...
        if total_invocations > 0:
            metrics["average_action_execution_time"] = (
