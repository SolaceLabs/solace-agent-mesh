...
def get_action_prompt(agent_name, action_dict, configs_added, api_description, action_file_content=None):
    prompt = f"""
{DEFAULT_ACTION_PURPOSE_PROMPT}

ACTION DETAILS TO BE IMPLEMENTED
Action Name: {action_dict['name']}
Action Description: {action_dict['description']}
Expected Return: {action_dict['returns']}
Current action file to be edited: 
{action_file_content}

The configs that were added are:
{configs_added}
...
# OUTPUT FORMAT
Remember to provide your response as a code block containing the complete action code. Always return the complete file, not just the changes.
It is assumed that the code is written in python, do not specify the language in the code block simply return it between ``` and ```.
"""
