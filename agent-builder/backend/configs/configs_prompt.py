...
def get_configs_prompt(agent_dict, api_key_required, configs_file_path):
    # Retrieve current configuration file content
    if configs_file_path and os.path.exists(configs_file_path):
        with open(configs_file_path, "r") as f:
            agent_configs_content = f.read()
    else:
        agent_configs_content = "No configuration file provided."
        print(f"Configuration file not found: {configs_file_path}")

    # Generate prompt
    prompt = f"""
{DEFAULT_CONFIGS_PURPOSE_PROMPT}

You need to modify the following configuration file for the {agent_dict['name']} agent.
Agent description: {agent_dict['description']}
GLOBAL CONTEXT: {agent_dict}
IS AN API KEY REQUIRED = {api_key_required}
The current configuration is file to be edited is :
{agent_configs_content}

{DEFAULT_CONFIGS_OUTPUT_PROMPT}
"""
    return prompt
...
DEFAULT_CONFIGS_OUTPUT_PROMPT = """
OUTPUT FORMAT:
Remember to provide your response as a JSON object with a  "file_content" field containing the complete YAML configuration, 
a "configs_added" field with just the names of the configs that were added as a list, and a api_key_name value which is the name of the
environment variable for the API KEY if it is required., or just put "" if not required
Always return the complete configuration file, not just the changes.
"""