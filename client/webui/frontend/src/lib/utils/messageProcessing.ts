...
export function checkHasVisibleContent(parts: Part[]): boolean {
    return parts.some(
        p => (p.kind === "text" && (p as TextPart).text.trim()) || p.kind === "file" || (p.kind === "data" && (p as DataPart).data && ((p as DataPart).data.type === "deep_research_progress" || (p as DataPart).data.type === "compaction_notification" || (p as DataPart).data.type === "redirect"))
    );
}
