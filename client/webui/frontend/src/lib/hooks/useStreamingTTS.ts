...
export function useStreamingTTS(options: UseStreamingTTSOptions): UseStreamingTTSReturn {
     ...
     */
    const generateStreamedAudio = useCallback(async (text: string): Promise<HTMLAudioElement | null> => {
        try {
     ...
     */
    const doProcessing = useCallback(async (text: string, isComplete: boolean) => {
     ...
     */
    const processStreamingText = useCallback(async (text: string, isComplete: boolean) => {
