...
export function useScheduledTasks() {
    ...
    const fetchTasks = useCallback(
        async (pageNumber: number = 1, pageSize: number = 20, enabledOnly: boolean = false, includeNamespaceTasks: boolean = true): Promise<ScheduledTaskListResponse | null> => {
            setIsLoading(true);
            setError(null);
            ...
            try {
                ...
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/?${params.toString()}`, { credentials: "include" });
    ...
    const fetchTask = useCallback(
        async (taskId: string): Promise<ScheduledTask | null> => {
            ...
            try {
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/${taskId}`, { credentials: "include" });
    ...
    const enableTask = useCallback(
        async (taskId: string): Promise<boolean> => {
            ...
            try {
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/${taskId}/enable`, {
                    method: "POST",
                    credentials: "include",
                });
    ...
    const disableTask = useCallback(
        async (taskId: string): Promise<boolean> => {
            ...
            try {
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/${taskId}/disable`, {
                    method: "POST",
                    credentials: "include",
                });
    ...
    const fetchExecutions = useCallback(
        async (taskId: string, pageNumber: number = 1, pageSize: number = 20): Promise<ExecutionListResponse | null> => {
            setIsLoading(true);
            setError(null);
            ...
            try {
                ...
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/${taskId}/executions?${params.toString()}`, { credentials: "include" });
    ...
    const fetchRecentExecutions = useCallback(
        async (limit: number = 50): Promise<ExecutionListResponse | null> => {
            ...
            try {
                const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/executions/recent?limit=${limit}`, { credentials: "include" });
    ...
    const fetchSchedulerStatus = useCallback(async (): Promise<SchedulerStatus | null> => {
        ...
        try {
            const response = await authenticatedFetch(`${apiPrefix}/scheduled-tasks/scheduler/status`, { credentials: "include" });
