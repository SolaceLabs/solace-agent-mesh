...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
    const { configWelcomeMessage: rawWelcomeMessage, persistenceEnabled, configCollectFeedback, backgroundTasksEnabled, backgroundTasksDefaultTimeoutMs, autoTitleGenerationEnabled, configUseAuthorization } = useConfigContext();
            ...
            async (taskId: string, taskSessionId: string) => {
                ...
                if (autoTitleGenerationEnabled) {
                    ...
                    if (taskSessionId && taskSessionId.trim() !== "" && !sessionsWithAutoGeneratedTitles.current.has(taskSessionId)) {
                        ...
                        try {
                            // Use the title-data endpoint which extracts messages from chat_tasks OR SSE buffer
                            // This works even when FE wasn't watching and chat_tasks weren't saved yet
                            console.debug("[ChatProvider] Fetching title-data for background task:", taskId);
                            const titleData = await api.webui.get(`/api/v1/tasks/${taskId}/title-data`);
                            console.debug("[ChatProvider] Title-data response:", titleData);
                            ...
                            if (userMessageText && agentResponseText) {
                                console.debug("[ChatProvider] Calling generateTitle for background task");
                                await generateTitle(taskSessionId, userMessageText, agentResponseText);
                            } else {
                                console.warn("[ChatProvider] Missing messages for title generation:", {
                                    hasUser: !!userMessageText,
                                    hasAgent: !!agentResponseText,
                                });
                            }
                        } catch (error) {
    ...
    const handleSseMessage = useCallback(
        (event: MessageEvent) => {
            ...
            setMessages(prevMessages => {
                ...
                console.log(`[TitleDebug] Event check: isFinalEvent=${isFinalEvent}, currentTaskIdFromResult=${currentTaskIdFromResult}, isReplaying=${isReplayingEventsRef.current}`);
                if (isFinalEvent && currentTaskIdFromResult && !isReplayingEventsRef.current) {
                    ...
                    console.log(`[TitleDebug] taskMessagesToSave.length=${taskMessagesToSave.length}, user msgs=${taskMessagesToSave.filter(m => m.isUser).length}, agent msgs=${taskMessagesToSave.filter(m => !m.isUser).length}`);
                    if (taskMessagesToSave.length > 0) {
                        ...
                        saveTaskToBackend(
                            ...
                            .then(async saved => {
                                console.log(`[TitleDebug] saveTaskToBackend resolved, saved=${saved}, taskSessionId=${taskSessionId}, taskId=${currentTaskIdFromResult}`);
                                if (saved) {
                                    if (typeof window !== "undefined") {
                                        window.dispatchEvent(new CustomEvent("new-chat-session"));
                                    }
                                ...
                                const alreadyGenerated = sessionsWithAutoGeneratedTitles.current.has(taskSessionId);
                                console.log(`[TitleDebug] Title check: taskSessionId=${taskSessionId}, alreadyGenerated=${alreadyGenerated}, autoTitleGenerationEnabled=${autoTitleGenerationEnabled}`);
                                if (taskSessionId && !alreadyGenerated) {
                                    if (autoTitleGenerationEnabled) {
                                        ...
                                        console.log(`[TitleDebug] userMessageText="${userMessageText?.substring(0, 50)}", agentResponseText="${agentResponseText?.substring(0, 50)}", agentMessage found=${!!agentMessage}, agentMessage parts=${agentMessage?.parts?.length}`);
                                        if (userMessageText && agentResponseText) {
                                            sessionsWithAutoGeneratedTitles.current.add(taskSessionId);
                                            console.log(`[TitleDebug] Calling generateTitle for session ${taskSessionId}`);
                                            ...
                                            console.warn(`[TitleDebug] Skipping generateTitle: missing userMessageText or agentResponseText`);
                                        }
                                    } else {
                                        console.log(`[TitleDebug] autoTitleGenerationEnabled is false, skipping`);
                                    }
                                } else if (!taskSessionId) {
                                    console.warn(`[TitleDebug] No taskSessionId, skipping title generation`);
                                }
                            })
                            .catch(error => {
    ...
    const handleNewSessionCore = useCallback(
        ...
        [isResponding, currentTaskId, selectedAgentName, isCancelling, closeCurrentEventSource, activeProject, setActiveProject, closePreview, isTaskRunningInBackground, setRagData, findAgentBubbleForTask, resetCollaborativeState]
    );
