...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
    ...
    // serializeChatMessage is now imported from @/lib/utils/serializeChatMessage
        ...
        (effect: ChatEffect) => {
            switch (effect.type) {
                ...
                case "save-task": {
                    ...
                    const taskMessagesToSave = taskMessages.filter(msg => msg.taskId === taskId && !msg.isStatusBubble);
                    if (taskMessagesToSave.length === 0) break;

                    const isBackgroundTask = isTaskRunningInBackground(taskId);
                    const messageBubbles = taskMessagesToSave.map(serializeChatMessage);
