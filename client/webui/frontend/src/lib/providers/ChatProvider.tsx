...
import { useConfigContext, useArtifacts, useAgentCards, useErrorDialog, useTitleGeneration, useBackgroundTaskMonitor } from "@/lib/hooks";
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
    const { configWelcomeMessage, configServerUrl, persistenceEnabled, configCollectFeedback, backgroundTasksEnabled, backgroundTasksDefaultTimeoutMs, autoTitleGenerationEnabled } = useConfigContext();
    ...
    const {
        ...
        onTaskCompleted: useCallback(
            async (taskId: string) => {
                ...
                if (autoTitleGenerationEnabled) {
                    ...
                    if (!taskSessionId || taskSessionId.trim() === "") {
                        try {
                            const statusResponse = await authenticatedFetch(`${apiPrefix}/tasks/${taskId}/status`);
                    ...
                    if (taskSessionId && taskSessionId.trim() !== "" && !sessionsWithAutoGeneratedTitles.current.has(taskSessionId)) {
                        ...
                        try {
                            ...
                            const taskData = await fetchJsonWithError(`${apiPrefix}/sessions/${taskSessionId}/chat-tasks`);
                            const tasks = taskData.tasks || [];

                            // Find the task that matches
                            const matchingTask = tasks.find((t: TaskFromAPI) => t.taskId === taskId);
                            if (matchingTask) {
            ...
            [addNotification, apiPrefix, autoTitleGenerationEnabled, generateTitle]
        ),
                            ...
                            (async () => {
                                ...
                                if (!userMessageText || !agentResponseText) {
                                    ...
                                    try {
                                        ...
                                        // Find the task that matches
                                        const matchingTask = tasks.find((t: TaskFromAPI) => t.taskId === currentTaskIdFromResult);
                    ...
                    for (const bgTask of sessionBackgroundTasks) {
                        ...
                        if (status && status.is_running) {
                            ...
                            if (autoTitleGenerationEnabled && newSessionId && !sessionsWithAutoGeneratedTitles.current.has(newSessionId)) {
                                ...
                                (async () => {
                                    try {
                                        const taskData = await fetchJsonWithError(`${apiPrefix}/sessions/${newSessionId}/chat-tasks`);
                                        const tasks = taskData.tasks || [];
                                        console.log(`[ChatProvider] Fetched ${tasks.length} tasks from database for title generation`);

                                        // Find the task that matches
                                        const matchingTask = tasks.find((t: TaskFromAPI) => t.taskId === bgTask.taskId);
                                        if (matchingTask) {
                                            const messageBubbles = JSON.parse(matchingTask.messageBubbles);
                                            let userMessageText = "";
                                            let agentResponseText = "";
    ...
    useEffect(() => {
        const handleTitleUpdated = async (event: Event) => {
            ...
            if (updatedSessionId === sessionId) {
                try {
                    const response = await authenticatedFetch(`${apiPrefix}/sessions/${updatedSessionId}`);
                    if (response.ok) {
