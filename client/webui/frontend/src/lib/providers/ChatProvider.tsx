...
import { useConfigContext, useArtifacts, useAgentCards, useErrorDialog, useTitleGeneration } from "@/lib/hooks";
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
                    ...
                    setMessages(currentMessages => {
                        ...
                        if (taskMessages.length > 0) {
                            ...
                            saveTaskToBackend({
                                ...
                                .then(() => {
                                    if (sessionId && !sessionsWithAutoGeneratedTitles.current.has(sessionId)) {
                                        // Check if session name is still "New Chat" or null (indicates first task)
                                        const needsTitle = !sessionName || sessionName === "New Chat";
                                        if (needsTitle) {
                                            ...
                                            if (userMessageText && agentResponseText) {
                                                console.log("[ChatProvider] Triggering automatic title generation for new session");
                                                // Mark this session as having had AUTOMATIC title generation
                                                sessionsWithAutoGeneratedTitles.current.add(sessionId);
                                                generateTitle(sessionId, userMessageText, agentResponseText).catch(error => {
                                ...
                                .catch(error => {
                                    console.error("[ChatProvider] Task save failed, skipping title generation:", error);
                                });
                        }
    ...
    const handleSubmit = useCallback(
        ...
        [sessionId, isResponding, isCancelling, selectedAgentName, closeCurrentEventSource, apiPrefix, uploadArtifactFile, saveTaskToBackend, serializeMessageBubble, activeProject, cleanupUploadedFiles, setError]
    );
