...
/**
 * Retrieves artifact content for a file in a session.
 * Uses the unified artifact utility with version "latest" to fetch actual file content.
 *
 * @param sessionId - The session ID containing the artifact
 * @param filename - The filename of the artifact
 * @returns Promise with content (base64) and mimeType
 */
export async function getArtifactContent(sessionId: string, filename: string): Promise<{ content: string; mimeType: string }> {
