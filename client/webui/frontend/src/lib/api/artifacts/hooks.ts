 ...
 *
 * @param sessionId - The session ID (null or empty string disables the query)
 * @param filename - The filename to fetch (null disables the query)
 * @returns React Query result with content and mimeType
 */
export function useArtifactContent(sessionId: string | null, filename: string | null) {
    const hasSession = !!sessionId && sessionId !== "";
    return useQuery({
        queryKey: hasSession && filename ? artifactKeys.content(sessionId, filename) : ["artifacts", "content", "empty"],
        queryFn: hasSession && filename ? () => artifactService.getArtifactContent(sessionId, filename) : skipToken,
        enabled: hasSession && !!filename,
