...
// Error label helper component
const ErrorLabel = ({ children }: { children: React.ReactNode }) => <div className="text-xs text-red-600 dark:text-red-400">{children}</div>;
            ...
            {fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-[1fr_1fr_auto] items-start gap-2">
                    <div>
                        ...
                        <input {...register(`${name}.${index}.key`)} type="text" className="w-full border px-3 py-2" />
                    </div>
                    <div>
                        {index === 0 && <div className="mb-1 text-sm">Value</div>}
                        <input {...register(`${name}.${index}.value`)} type="text" className="w-full border px-3 py-2" />
                    </div>
