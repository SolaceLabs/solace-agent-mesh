...
const RadioIndicator: React.FC<{ selected: boolean }> = ({ selected }) => (
    <div className="relative flex h-3.5 w-3.5 items-center justify-center">
        <div className={`h-3.5 w-3.5 rounded-full border-2 transition-colors ${selected ? "border-blue-600 dark:border-blue-400" : "border-gray-300 dark:border-gray-600"}`}>
            {selected && <div className="absolute inset-0 m-[3px] rounded-full bg-blue-600 dark:bg-blue-400" />}
        </div>
    </div>
);
...
export const ToolsSelector: React.FC<ToolsSelectorProps> = ({
    ...
    const hasDeepResearch = agents.some(a => a.skills?.some(s => s.id === "deep_research"));

    // Check if web search is available
    const hasWebSearch = agents.some(a => a.skills?.some(s => s.id === "web_search"));
    ...
    return (
        <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
                <Button variant={buttonVariant} size="sm" disabled={disabled} className="gap-1.5" tooltip="Research Tools">
                    {deepResearchEnabled ? <Microscope className="h-4 w-4" /> : webSearchEnabled ? <Globe className="h-4 w-4" /> : <Wrench className="h-4 w-4" />}
                    {hasActiveTools && <span className="text-xs">{deepResearchEnabled ? "Deep Research" : "Web Search"}</span>}
                </Button>
            </DropdownMenuTrigger>
                ...
                {hasWebSearch && (
                    <Tooltip delayDuration={0}>
                        ...
                        <TooltipContent side="right" className="max-w-xs">
                            {webSearchConfigured === false ? <p className="text-xs">Requires TAVILY_API_KEY or GOOGLE_SEARCH_API_KEY + GOOGLE_CSE_ID environment variables</p> : <p className="text-xs">Search the web for current information</p>}
                        </TooltipContent>
                    </Tooltip>
                )}
                    ...
                    <>
                        ...
                        {deepResearchEnabled && onDeepResearchSettingsClick && (
                            <DropdownMenuItem
                                ...
                                className="flex cursor-pointer items-center gap-2 pl-8"
                            >
