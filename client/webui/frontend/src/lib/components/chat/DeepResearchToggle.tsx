...
export const DeepResearchToggle: React.FC<DeepResearchToggleProps> = ({ enabled, onToggle, disabled, agents, onSettingsClick }) => {
    // Find agent with deep_research skill
    const deepResearchAgent = agents.find(a => a.skills?.some(s => s.id === "deep_research"));
    ...
    return (
        <Tooltip>
            <TooltipTrigger asChild>
                <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium transition-all ${
                        enabled ? "bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400"
                    } ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"} `}
                    aria-label={enabled ? "Deep Research enabled - Click to disable" : "Enable Deep Research"}
                >
            ...
            <TooltipContent>{enabled ? "Deep Research enabled - Click to disable" : "Enable Deep Research"}</TooltipContent>
        </Tooltip>
    );
};
