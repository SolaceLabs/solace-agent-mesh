...
const GenericToolNode: React.FC<NodeProps<GenericToolNodeType>> = ({ data, id }) => {
    ...
    return (
        ...
        >
            <Handle type="target" position={Position.Left} id={`${id}-tool-left-input`} className="!bg-cyan-500" isConnectable={false} style={{ top: "25%" }} />
            ...
            <Handle type="source" position={Position.Left} id={`${id}-tool-bottom-output`} className="!bg-cyan-500" isConnectable={false} style={{ top: "75%" }} />
            <Handle type="source" position={Position.Right} id={`${id}-tool-right-output-artifact`} className="!bg-cyan-500" isConnectable={false} style={{ top: "50%" }} />
        </div>
    );
};
