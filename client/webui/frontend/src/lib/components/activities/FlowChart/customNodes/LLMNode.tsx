...
const LLMNode: React.FC<NodeProps<LLMNodeType>> = ({ data }) => {
    ...
    return (
        ...
        >
            <Handle type="target" position={Position.Left} id="llm-left-input" className="!bg-teal-500" isConnectable={false} style={{ top: "25%" }} />
            <div className="flex items-center justify-center">
                <div className={`mr-2 h-2 w-2 rounded-full ${getStatusColor()}`} />
                <div className="text-md">{data.label}</div>
            </div>
            <Handle type="source" position={Position.Left} id="llm-bottom-output" className="!bg-teal-500" isConnectable={false} style={{ top: "75%" }} />
        </div>
    );
};
