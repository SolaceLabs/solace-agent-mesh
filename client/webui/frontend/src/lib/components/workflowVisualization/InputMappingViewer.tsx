    ...
    if (typeof value === "string") {
        ...
        return (
            <span className="inline-flex items-end gap-1">
                <span className="font-mono text-(--color-error-w100)">"{value}"</span>
                {hasNodeRefs && onNavigateToNode && (
                    <Button variant="ghost" size="icon" onClick={handleNavigate} onMouseEnter={() => onHighlightNodes?.([nodeRefs[0]])} onMouseLeave={() => onHighlightNodes?.([])} tooltip={`Navigate to ${nodeRefs[0]}`}>
