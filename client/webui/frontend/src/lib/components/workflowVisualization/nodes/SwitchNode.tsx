    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-sm text-(--secondary-text-wMain)">{totalCases} cases</span>
                </div>
            </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        {cases.map((caseItem: { condition?: string }, index: number) => (
                            <div key={index} className="flex items-center gap-2">
                                <span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded border border-(--secondary-w20) text-xs font-medium text-(--secondary-text-wMain)">{index + 1}</span>
                                <span className="block truncate rounded bg-(--secondary-w10) px-2 py-1 text-sm text-(--secondary-text-wMain)" title={caseItem.condition}>
                                    {caseItem.condition || ""}
                                </span>
                            </div>
                        ))}
                        {hasDefault && (
                            <div className="flex items-center gap-2">
                                <span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded border border-(--secondary-w20) text-xs font-medium">{cases.length + 1}</span>
                                <span className="flex-1 rounded bg-(--secondary-w10) px-2 py-1 text-sm text-(--secondary-text-wMain)">default</span>
                            </div>
                        )}
                    </div>
                </div>
