...
import { Volume2, AlertCircle, Play, Loader2 } from "lucide-react";
import { useAudioSettings, useConfigContext } from "@/lib/hooks";
import { Label, Switch, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Input, Button, Dialog, DialogContent, DialogDescription, DialogTitle, VisuallyHidden } from "@/lib/components/ui";
...
export const TTSSettingsDialog: React.FC<TTSSettingsDialogProps> = ({ open, onOpenChange }) => {
    ...
    // Check TTS configuration status and auto-reset provider if needed
    useEffect(() => {
        if (!open) return;
        ...
        checkConfig();
    }, [open, settings.ttsProvider, updateSetting]);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md" showCloseButton={true}>
                <VisuallyHidden>
                    <DialogTitle>Text to Voice Settings</DialogTitle>
                    <DialogDescription>Configure text-to-speech settings</DialogDescription>
                </VisuallyHidden>

                <div className="space-y-4">
                    <div className="flex items-center gap-2 border-b pb-2">
                        <Volume2 className="size-5" />
                        <h3 className="text-lg font-semibold">Text to Voice</h3>
                    </div>
