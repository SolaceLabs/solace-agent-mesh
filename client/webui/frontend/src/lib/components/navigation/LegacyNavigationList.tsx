...
import { LegacyNavigationButton } from "@/lib/components/navigation/LegacyNavigationButton";
...
export const LegacyNavigationList: React.FC<NavigationListProps> = ({ items, bottomItems, activeItem, onItemClick }) => {
    ...
    return (
        <nav className="flex flex-1 flex-col" role="navigation" aria-label="Main navigation">
            ...
            <ul className="space-y-1">
                {items.map(item => (
                    <li key={item.id}>
                        <LegacyNavigationButton item={item} isActive={activeItem === item.id} onItemClick={onItemClick} />
                ...
                {bottomItems &&
                    ...
                    bottomItems.map(item => (
                        <li key={item.id} className="my-4">
                            <LegacyNavigationButton key={item.id} item={item} isActive={activeItem === item.id} onItemClick={onItemClick} />
                        </li>
                    ))}
                {/* User or Settings */}
                {logoutEnabled ? (
